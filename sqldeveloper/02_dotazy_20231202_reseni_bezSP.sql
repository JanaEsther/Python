...
CREATE OR REPLACE VIEW ProdejeView AS
SELECT c.*, s.productid, s.calendarid, s.units, s.revenue
FROM Cities2 c
JOIN SQL1.sales s
ON c.zip = s.zip
...
/* SAMOSTATNÁ PRÁCE */
-- Rozděl produkty podle ceny od 0 do 900 po 300 do kategorií 300(0-300), 600(300-600), 900(600-900), 
--a nad 900 a zjisti kolik výrobků je v které kategorii

select CASE WHEN Price < 300 THEN 'cheap'
            WHEN Price < 600 THEN 'middle'
            WHEN price > 900 THEN 'expansive'
        ELSE 'overpriced' 
        
       END AS price_category, count(*) as pocet, min(Price), max(Price)     
FROM SQL1.Products
GROUP BY CASE WHEN Price < 300 THEN 'cheap'
              WHEN Price < 600 THEN 'middle'
              WHEN price > 900 THEN 'expansive'
       END;
